    ...
    if is_admin {
        match cmd {
            AdminCommand::MakeAdmin => {
                let _: () = redis_conn.sadd(user_id.to_string() + ":admin_chats", chat_id.0)
                    .expect("Failed to add chat to admin_chats");

                let bot_chats: Vec<i64> = redis_conn.smembers(user_id.to_string() + ":bot_chats")
                ...
                bot.send_message(chat_id, "Admin chat registered! Please select chats to moderate:")
            ...
            AdminCommand::Enable { feature } => {
                bot.send_message(chat_id, format!("Feature '{}' enabled.", feature)).await?;
            }
            AdminCommand::Disable { feature } => {
                bot.send_message(chat_id, format!("Feature '{}' disabled.", feature)).await?;
            }
            AdminCommand::Stats => {
                bot.send_message(chat_id, "Stats: [Placeholder]").await?;
            }
            AdminCommand::Reputation { user } => {
                bot.send_message(chat_id, format!("Reputation for {}: [Placeholder]", user)).await?;
            }
            AdminCommand::Recent => {
                bot.send_message(chat_id, "Recent flagged messages: [Placeholder]").await?;
            }
            AdminCommand::AddRegex { pattern } => {
                bot.send_message(chat_id, format!("Added regex pattern: {}", pattern)).await?;
            }
        }
    } else {
